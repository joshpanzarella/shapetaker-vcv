...
Both oscillator cores can be crossfaded using CV and pushed to the outer edge of your speakers using the **WIDTH** knob. Widening works by amplifying the sides while driving the Formant section’s motion progressively further out of phase.
...
- **DEPTH:** how far the modulator bends the curve within each waveform cycle. At zero the waveform is traditional. As depth rises, a formant-like sweep opens up inside every note. The lower quarter of the knob is subtle by nature; the voice truly clears its throat from the midpoint onward. An envelope into DEPTH CV is the most direct way to make each note speak as it sounds.
- **RATIO (Knob):** The modulator's speed as a multiple of the oscillator's pitch, snapped to eight musical ratios. High integers (×5, ×7) turn metallic and bell-like; ×0.5 repeats its pattern every *two* cycles, adding a growling octave-down component.
...
- **XSYNC:** When engaged, Core V hard-resets the phase of Core Z on each cycle. Modulating Core Z's pitch while active produces aggressive, tearing harmonic sweeps.
- **REV. SYNC:** The signature misbehavior.
...
1. **Sigmoid Saw:** The morphing sigmoid waveform described throughout this manual. All Formant section controls operate as documented above.
...
1. **Equal Power:** A standard centered blend with consistent apparent volume.
...
- **Where the magic lives:** The Formant Section is pitch-locked, so its character survives transposition — but it is still shaped by SHAPE. If DEPTH seems to be doing little, raise the Shape controls toward their middle and upper range; a pure sawtooth gives the engine nothing to bend.
